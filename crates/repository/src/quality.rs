...
mod tests {
    ...
    fn renders_rust_js_php_hooks_without_package_scripts() {
        ...
        assert!(
            config.contains("cd \\\"workspace/app\\\" && oxfmt --config .oxfmtrc.json --check .")
        );
        assert!(config.contains("cd \\\"workspace/app\\\" && vitest run --config vitest.config.ts"));
        assert!(config.contains("composer exec rector -- --config rector.php --dry-run"));
