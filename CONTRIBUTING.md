...
Build output is intentionally local to `.cache` through `.cargo/config.toml`. Do not wrap normal Cargo commands with `rust-cache run`. Use `rust-cache run` for Tauri Android or other Gradle-based commands so mobile project build output is redirected into `.cache/android/builds`.
