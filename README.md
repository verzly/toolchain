...
No `.taplo.toml`, `.vscode/settings.json`, or repository-local schema mapping is required. The schema URL lives directly in `datarose.toml` as a leading `#:schema` directive, while `verzly repository check` validates the same configuration offline through the executable.

The schema URL is pinned to the toolchain release tag, not to `master`. A project generated by `verzly` v0.4.0 points to the v0.4.0 schema. When `github-release prepare` bumps the toolchain version, it also updates the schema references in `datarose.toml`, the README, and the public schema file.
